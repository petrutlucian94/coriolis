...
SUSE_DISTRO_IDENTIFIERS = ['sles', 'opensuse-leap', 'opensuse-tumbleweed', 'opensuse']
...
class SUSEOSMountTools(base.BaseLinuxOSMountTools):
    ...
    def _allow_ssh_env_vars(self):
        if not utils.test_ssh_path(self._ssh, SSHD_CONFIG_PATH):
            self._exec_cmd("sudo cp %s %s" % (USR_SSHD_CONFIG_PATH, SSHD_CONFIG_PATH))
        self._exec_cmd('sudo sed -i -e "\\$aAcceptEnv *" %s' % SSHD_CONFIG_PATH)
        try:
    ...
    def setup(self):
        ...
        retry_ssh_cmd("zypper --non-interactive install psmisc cryptsetup")
        ...
        try:
            ...
            retry_ssh_cmd("zypper --non-interactive install lvm2")
