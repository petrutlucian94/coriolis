...
class BaseSSHOSMountTools(BaseOSMountTools):
    ...
    def _exec_cmd(self, cmd, timeout=None):
        ...
        try:
            return utils.exec_ssh_cmd(
                self._ssh, cmd, self._environment, get_pty=False, timeout=timeout
            )
    ...
    def _exec_sudo_env_cmd(self, cmd, timeout=None):
        ...
        if not timeout:
            timeout = self._osmount_operation_timeout
        env_cmd = "sudo %s%s" % (utils.get_env_command_prefix(self._environment), cmd)
        try:
            return utils.exec_ssh_cmd(
                self._ssh,
                env_cmd,
...
class BaseLinuxOSMountTools(luks_mixin.LinuxLUKSMixin, BaseSSHOSMountTools):
    ...
    def _get_mounted_devices(self):
        ...
        block_devs = self._exec_cmd("ls -al /dev | grep ^b || true").splitlines()
    ...
    def _mask_minion_efi_firmware(self, os_root_dir):
        ...
        if firmware_type is None:
            firmware_type = self._osmorphing_info.get("osmorphing_parameters", {}).get(
                "firmware_type"
            )
        ...
        LOG.info(
            ...
            "OSMorphing chroot at '%s'",
            os_root_dir,
        )
        self._exec_cmd(
            "sudo mount -t tmpfs tmpfs %s" % os.path.join(os_root_dir, "sys/firmware")
        )
    ...
    def run_user_script(self, user_script):
        ...
        try:
            utils.exec_ssh_cmd(
                self._ssh, "sudo chmod +x %s" % script_path, get_pty=False
            )

            utils.exec_ssh_cmd(self._ssh, f'sudo "{script_path}"', get_pty=False)
        except Exception as err:
